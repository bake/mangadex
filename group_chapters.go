...
// group.
func (c *Client) GroupChapters(ctx context.Context, id string, opts *GroupChaptersOptions) ([]PreviewChapter, error) {
	values, err := query.Values(opts)
	if err != nil {
		return nil, errors.Wrap(err, "could not encode options")
	}
	cs, err := c.previewChapters(ctx, "/group/"+id+"/chapters", values)
	if err != nil {
