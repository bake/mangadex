...
func (c *Client) Manga(ctx context.Context, id int, opts *MangaOptions) (Manga, error) {
	...
	raw, err := c.get(ctx, fmt.Sprintf("/manga/%d", id), values)
	if err != nil {
		return Manga{}, errors.Wrapf(err, "could not get manga %d", id)
	}
	var res Manga
	if err := json.Unmarshal(raw, &res); err != nil {
		return Manga{}, errors.Wrapf(err, "could not unmarshal manga %d", id)
	}
	return res, nil
}
